...
![Внешний вид карточек](assets/template_appearence.png)

## Миграция старых карточек

В колоде могут находиться карты с разными типами, но при желании можно их свести к одному

В меню Browse нужно выбрать нужные карты (например по колоде или по типу карты), затем нажать
...
Далее выбраем тип карт `bgunwest_withlove...`
и сопоставляем поля старого и нового типа карт примерно вот так:
...
## Настройка колоды
> Если честно мне было впадлу тут распинаться при желании погуглить можно про параметры эти
...
Важный параметр `desired retention`, от него напрямую зависит число повторений.
Нормально в промежутке 0.75-0.90

> Retention - число обозначающее процент карточек ты "знаешь" из колоды

### Другие важные настройки
![new cards](assets/new_cards.png)

- learning steps -- я юзаю (30s 1m 5m 1h) на скрине неправильно

Бля погугли че это мне впадлу расписывать но там очевидно

![](assets/laspes_leeches.png)
- тут relearning steps -- тоже самое что для learning steps, но для переучиываемых
- leech freshold -- сколько раз можно проебать карточку перед тем как она либо исключится из колоды, либо анки ее просто тегнет (зависит от leech action)
