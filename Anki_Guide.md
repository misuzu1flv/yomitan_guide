...
вот этот ебать ахуенный темплейт: [качать](https://github.com/misuzu1flv/yomitan_guide/blob/main/assets/bigguns_for_bexoy_withLOVE333.apkg)

После запуска в анки добавится новая колода с новым типом карточек, эту колоду можно удалить, а можно добавлять новые карточки прямо в неё.

## Миграция карточек старых

Если хочется можно мигрировать старые карточки в новый тип карточек (они могут сосуществовать в одной колоде и с разным типом)

В card browser нужно выбрать нужные карты (например по колоде или по типу карты), затем нажать
